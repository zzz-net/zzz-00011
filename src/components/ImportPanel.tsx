...
import { Upload, FileText, Check, AlertTriangle, X, Package, Thermometer, ClipboardList, Database } from 'lucide-react';
import { useAppStore } from '@/store';
import type { FC, ChangeEvent, DragEvent } from 'react';
import type { FileType, ImportResult, FieldMapping, FieldMappingPreview } from '@/types';
import { format } from 'date-fns';
import { cn } from '@/lib/utils';
import FieldMappingPreviewModal from '@/components/FieldMappingPreviewModal';
...
interface ImportCardProps {
  ...
  accentColor: string;
  onImport: (file: File, fieldMappings?: FieldMapping[]) => Promise<ImportResult>;
  onPreview: (file: File, fileType: FileType) => Promise<FieldMappingPreview>;
  requiredColumns: string[];
}

const ImportCard: FC<ImportCardProps> = ({ title, description, fileType, icon: Icon, accentColor, onImport, onPreview, requiredColumns }) => {
  ...
  const handleConfirmImport = async (mappings: FieldMapping[]) => {
    if (!pendingFile) return;
    setLoading(true);
    setResult(null);
    try {
      const r = await onImport(pendingFile, mappings);
      setResult(r);
    } catch (err) {
  ...
  const handleFile = async (file: File) => {
    ...
    setResult(null);
    try {
      const preview = await onPreview(file, fileType);
          ...
          {loading ? (
            ...
            <>
              <div className="flex items-center gap-1.5">
                ...
                <Database className="h-4 w-4 text-slate-500 transition group-hover:text-purple-400" />
              </div>
              <span className="mt-1 text-sm text-slate-300">
                点击或拖拽 <span className="text-sky-400">.csv</span> 文件到此处
              </span>
              <span className="mt-0.5 text-[10.5px] text-slate-500">导入前将显示字段映射预览，支持中文表头自动匹配</span>
            </>
          )}
        </div>
...
const ImportPanel: FC = () => {
  const { importArrivals, importTemperatureLogs, importManualReviews, previewFieldMapping } = useAppStore();
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">CSV UTF-8 编码 · 支持中文表头自动识别与字段映射</p>
        </div>
        <span className="text-[11px] text-slate-500">字段映射自动保存至本地</span>
      </div>
